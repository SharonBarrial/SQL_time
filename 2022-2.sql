...
create function count_actors(@genre varchar(50), @year int)
returns table
(titulo_movie varchar(255),anio int, cantidad_actores int) 
as
return
	(SELECT m.title as titulo_movie, m.year as anio, 
	COUNT(mc.actor_id) AS cantidad_actores
	FROM movie_cast mc
	JOIN movies m ON mc.movie_id=m.id
	JOIN genres g ON m.genre_id=g.id
	WHERE g.description= @genre AND m.year= @year
